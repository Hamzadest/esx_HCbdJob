-----------------------------------------
-- Créé par Hamza#8602 pour Afterlife & Animus
-- Contact devafterlife@gmail.com ou contact@afterlife.fr
-----------------------------------------
...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			align    = 'top-left',
			elements = {
				{label = _U('cbd_clothes_civil'), value = 'citizen_wear'},
				{label = _U('cbd_clothes_cbd'), value = 'cbdplant_wear'},
			},
		}, function(data, menu)
			...
			if data.current.value == 'cbdplant_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'cbdplant_actions_menu'
...
function OpenCbdplantActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cbdplant_actions', {
			title    = 'Cbdplant',
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'cbdplant', function(data, menu)
					menu.close()
				end)
			end
		end, function(data, menu)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'cbdplant', vehicleProps)
				end, function(data, menu)
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'Burrito'},
		}
...
function OpenMobileCbdplantActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_cbdplant_actions', {
			title    = 'cbdplant',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cbdplant', 'Cbdprod', amount)
								end)
							end
						end
					end, function(data, menu)
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_hamzacdbjob:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_hamzacdbjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_hamzacdbjob:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_hamzacdbjob:putStockItems', itemName, count)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
...
AddEventHandler('esx_hamzacdbjob:hasEnteredMarker', function(zone)
	if zone == 'CbdbrutFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant'  then
		CurrentAction     = 'cbdbrut_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementCbd' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant'  then
		CurrentAction     = 'cbd_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementLimonade' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant'  then
		CurrentAction     = 'limonade_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'VenteCbd' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'CbdprodActions' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
		CurrentAction     = 'cbdplant_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
...
AddEventHandler('esx_hamzacdbjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'CbdbrutFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
		TriggerServerEvent('esx_hamzacdbjob:stopHarvest')
	end  
	if (zone == 'TraitementCbd' or zone == 'TraitementLimonade') and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
		TriggerServerEvent('esx_hamzacdbjob:stopTransform')
	end
	if (zone == 'VenteCbd') and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
		TriggerServerEvent('esx_hamzacdbjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.CbdprodActions.Pos.x, Config.Zones.CbdprodActions.Pos.y, Config.Zones.CbdprodActions.Pos.z)
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_hamzacdbjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_hamzacdbjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'cbdbrut_harvest' then
					TriggerServerEvent('esx_hamzacdbjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'limonade_traitement' then
					TriggerServerEvent('esx_hamzacdbjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'cbd_traitement' then
					TriggerServerEvent('esx_hamzacdbjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_hamzacdbjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'cbdplant_actions_menu' then
					OpenCbdplantActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'cbdplant', vehicleProps)
					end
		...
		if IsControlPressed(0,  167) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'cbdplant' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileCbdplantActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)